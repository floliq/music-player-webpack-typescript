    ...
    <li class="tracks__item flex">
        ...
        <div class="tracks__item__name">
            ...
            <h3 class="track__name">
                <a class="track__name__link" href="#">${item.title}</a>
            </h3>
            <span class="track__author">${item.artist}</span>
            </div>
        </div>
        <div class="tracks__item__albom">${item.album}</div>
        <div class="tracks__item__data flex">
            <span class="data__text">${getAddedDate(item.addedDate)}</span>
            <button class="track__like-btn ${item.isLiked ? 'like-btn--active' : ''}">
            <svg
                width="22"
                height="18"
                viewBox="0 0 22 18"
        ...
        <time class="tracks__item__time">${getTrackDuration(item.duration)}</time>
